...
We split into pairs to research both candidates. The final theme was decided — not the one I'd been assigned. I ended up owning a supporting dataset: **monthly heatwave and cold wave statistics from Korean public records.**
...
| Format | CSV |
| Volume | Visible at a glance — few rows, few columns |
| Engineering required | None |

I pushed to extract something useful — tracked down disease and mortality figures linked to extreme weather events to surface as big-number metrics. The data barely supported it. In the end, my pipeline looked like this:
...
Part of that was on me. As team lead, I had overcorrected from the 1st project — prioritizing open discussion to the point where decisions kept getting deferred.
...
My contribution represented less than 10% of what was on screen — and that's being generous.
...
| "How should data flow from source to warehouse — what needs to happen along the way?" | High engagement |
